...
def write_file(filepath, objects, depsgraph, scene,
    ...
    with ProgressReportSubstep(progress, 2, "OBJ Export path: %r" % filepath, "OBJ Export Finished") as subprogress1:
        with open(filepath, "w", encoding="utf8", newline="\n") as f:
            ...
            totverts = totuvco = totuvlmapco = totno = 1
            ...
            for i, ob_main in enumerate(objects):
                ...
                for ob, ob_mat in obs:
                    with ProgressReportSubstep(subprogress1, 6) as subprogress2:
                        uv_unique_count = uv_unique_count_lmap = no_unique_count = 0
                        ...
                        if EXPORT_UV:
                            ...
                            if faceuv:
                                ...
                                #     if uv.name == "UVMap_Lightmap":
                                #         uv_layer_lmap = uv.data[:]
                                #         has_lmapuv = True
                        else:
                            faceuv = False
                        ...
                        # Check if the mesh has vertex colour groups
                        has_colors = True
                        if me.vertex_colors:
                            # print(f"MESH {me.name} HAS COLOUR");
                            active_col_layer = me.vertex_colors.active.data
                        else:
                            has_colors = False

                        # Vert
                        # PLEASE work PLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASEPLEASE
                        for i in range(len(me_verts)):
                            v = me_verts[i]
                            if has_colors:
                                for poly in me.polygons:
                                    for loop in poly.loop_indices:
                                        if loops[loop].vertex_index == i:
                                            vc = active_col_layer[loop].color
                                if vc:
                                    fw('v %.6f %.6f %.6f %.6f %.6f %.6f %.6f\n' % (v.co[:]+vc[:]))
                                else:
                                    fw('v %.6f %.6f %.6f\n' % v.co[:])
                            else:
                                fw('v %.6f %.6f %.6f\n' % v.co[:])
                        ...
                        if faceuv:
                            ...
                            # uv_face_mapping_lmap = [None] * len(face_index_pairs)

                            uv_dict = {}
                            # uv_dict_lmap = {}

                            uv_get = uv_dict.get
                            # uv_get_lmap = uv_dict_lmap.get
                            for f, f_index in face_index_pairs:
                                uv_ls = uv_face_mapping[f_index] = []
                                # uv_ls_lmap = uv_face_mapping_lmap[f_index] = []
                                for uv_index, l_index in enumerate(f.loop_indices):
                                    uv = uv_layer[l_index].uv
                                    # uv_lmap = uv_layer_lmap[l_index].uv
                                    ...
                                    # uv_key_lmap = loops[l_index].vertex_index, veckey2d(uv_lmap)

                                    uv_val = uv_get(uv_key)
                                    # uv_val_lmap = uv_get_lmap(uv_key_lmap)
                                    if uv_val is None:
                                        uv_val = uv_dict[uv_key] = uv_unique_count
                                        fw('vt %.6f %.6f\n' % uv[:])
                                        uv_unique_count += 1
                                    # if uv_val_lmap is None:
                                    #     uv_val_lmap = uv_dict_lmap[uv_key_lmap] = uv_unique_count_lmap
                                    #     fw('vt_lmap %.6f %.6f\n' % uv_lmap[:])
                                    #     uv_unique_count_lmap += 1
                                    uv_ls.append(uv_val)
                                    # uv_ls_lmap.append(uv_val_lmap)

                            del uv_dict, uv, f_index, uv_index, uv_ls, uv_get, uv_key, uv_val
                            # del uv_dict_lmap, uv_lmap, uv_ls_lmap, uv_get_lmap, uv_key_lmap, uv_val_lmap
